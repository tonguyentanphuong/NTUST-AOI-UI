...
const getApiBaseUrl = () => {
    if (typeof window !== 'undefined') {
        const savedUrl = localStorage.getItem('AOI_API_URL');
...
const getImgBaseUrl = () => {
    if (typeof window !== 'undefined') {
        const savedUrl = localStorage.getItem('AOI_IMG_URL');
        if (savedUrl) return savedUrl;
    }
    return import.meta.env.VITE_IMG_BASE_URL || 'http://localhost:8080/images';
};
...
export const inspectionService = {
    ...
    setConfig: (apiUrl: string, imgUrl: string) => {
        localStorage.setItem('AOI_API_URL', apiUrl);
        localStorage.setItem('AOI_IMG_URL', imgUrl);
        window.location.reload(); // Reload to apply
    },

    resetConfig: () => {
        localStorage.removeItem('AOI_API_URL');
        localStorage.removeItem('AOI_IMG_URL');
